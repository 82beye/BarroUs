...
export default function GraphView({ nodes, edges }: Props) {
  ...
  const [panelOpen, setPanelOpen] = useState(false); // 첫 진입엔 패널 닫혀 있음 — canvas 전체
  ...
  return (
    <div className="relative h-full w-full overflow-hidden">
      ...
      <button
        ...
        className="absolute right-3 top-3 z-20 border border-line bg-bg/90 px-3 py-1.5 text-[10px] uppercase tracking-[0.14em] backdrop-blur hover:bg-ink hover:text-bg"
      >
      ...
      {/* SELECTED NODE PANEL */}
      {selected ? (
        <div className="absolute left-3 top-3 z-20 max-w-[300px] border border-line bg-bg/90 p-4 backdrop-blur">
      ...
      {/* SIDEBAR — absolute drawer (canvas를 가리지 않고 floating) */}
      {panelOpen ? (
        <div className="absolute right-0 top-0 z-10 h-full w-full max-w-[320px] shadow-2xl">
          <GraphSettingsPanel
            force={force}
            setForce={setForce}
