...
// — 로직(서버 페치/리다이렉트)은 기존 그대로, 마크업만 매거진 톤으로 교체.
    ...
    <main className="mx-auto flex min-h-screen max-w-[1280px] flex-col px-5 pb-16 md:px-10">
      ...
      <div className="grid grid-cols-[1fr_auto_1fr] items-end gap-6 border-b border-line pt-6 pb-3.5 md:pt-8">
        <div className="flex gap-4 text-[11px] uppercase tracking-[0.12em]">
          ...
          <a href="#" className="text-muted hover:text-accent">임포트</a>
          <a href="#" className="text-muted hover:text-accent">공유</a>
        </div>
      ...
      {imported.length === 0 ? (
        ...
        <ul className="grid grid-cols-2 gap-5 border-t border-line pt-5 md:grid-cols-12">
          {imported.map((p, i) => {
            // 매거진 그리드 리듬: 6/3/3, 4/4/4, 12, …
            const rhythm = ["md:col-span-6", "md:col-span-3", "md:col-span-3", "md:col-span-4", "md:col-span-4", "md:col-span-4"];
              ...
              <li key={p.nodeId} className={`col-span-1 ${colSpan}`}>
                <Link href={`/dashboard/playlist/${p.nodeId}`} className="group block">
                  ...
                  >
                    ...
                    <div
                      className={`absolute left-3 top-2.5 font-mono text-[10px] ${p.isLikedSongs ? "text-accent" : "text-muted"}`}
                    >
                  ...
                  <div className="mt-2 grid grid-cols-[1fr_auto] items-baseline gap-x-3 gap-y-1 border-t border-line pt-2">
                    ...
                    <div className="font-mono text-[10px] text-muted">{p.trackCount}</div>
